...
// 
// - Убрать todo fixit 4real и прочий мусор
// - написать коменты
// 
// - отделить нахождение того, что надо красить от самой закраски
//      потому что оно иначе красит по порядку проверки и это затратно + зря + видно + накладывается неправильно на то,
//      что так то видно должно быть по-хорошему
//      иначе у меня художник получается, а не буфер
